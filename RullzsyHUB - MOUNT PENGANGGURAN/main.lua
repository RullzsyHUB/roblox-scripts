...
local Window = Rayfield:CreateWindow({
   Name = "RullzsyHUB | MOUNT PENGANGGURAN",
   Icon = "braces",
...
local jsonFolder = mainFolder .. "/json_mount_pengangguran"
...
local baseURL = "https://raw.githubusercontent.com/RullzsyHUB/roblox-scripts-json/refs/heads/main/json_mount_pengangguran/"
local jsonFiles = {
    "spawnpoint.json",
    "checkpoint_1.json",
    "checkpoint_2.json",
...
-- ✅ ENHANCED: Auto Walk Start-to-End dengan looping + auto jalan ke titik awal checkpoint
local function startAutoWalkSequence()
    local ok = EnsureJsonFile(jsonFiles[1])
    if not ok then
        Rayfield:Notify({
            Title = "Error",
            Content = "Gagal memuat checkpoint data dari server!",
            Duration = 4,
            Image = "ban"
        })
        return
    ...
    if firstData then
        ...
        if not isNear then
            Rayfield:Notify({
                ...
                Content = string.format("Terlalu jauh dari spawnpoint!\nJarak: %.1fm (Max: %d)", distance, maxDistanceToCheckpoint),
                Duration = 5,
        ...
        if currentCheckpoint > #jsonFiles then
            ...
            if loopingEnabled then
                Rayfield:Notify({
                    ...
                    Content = "Semua checkpoint selesai! Balik ke spawnpoint...",
                    Duration = 4,
                    Image = "repeat"
                })
                task.wait(2)
                currentCheckpoint = 1
                moveToCheckpoint("spawnpoint.json", 1)
                return
            else
                autoLoopEnabled = false
                Rayfield:Notify({
                    Title = "Auto Walk",
                    Content = "Auto walk selesai! Semua checkpoint dilewati.",
                    Duration = 4,
                    Image = "check-check"
                })
                return
        ...
        if data and #data > 0 then
            ...
            if not isNear then
                Rayfield:Notify({
                    ...
                    Content = string.format("Menuju titik awal %s (%.1fm)", checkpointFile, distance),
                    Duration = 4,
                    Image = "map-pin"
                })
                ...
                walkToStartingPoint(startPos, function(success)
                    if success then
                        Rayfield:Notify({
                            Title = "Auto Walk",
                            Content = "Mulai playback: "..checkpointFile,
                            Duration = 2,
                            Image = "check"
                        })
                        ...
                        Rayfield:Notify({
                            Title = "Auto Walk",
                            Content = "Gagal menuju titik awal!",
                            Duration = 3,
                            Image = "ban"
                        })
                        autoLoopEnabled = false
                ...
                -- Sudah dekat, langsung mulai playback
            ...
            Rayfield:Notify({
                Title = "Error",
                Content = "Gagal memuat data: " .. checkpointFile,
                Duration = 4,
                Image = "ban"
            })
            autoLoopEnabled = false
...
function moveToCheckpoint(fileName, index)
    local ok = EnsureJsonFile(fileName)
    if not ok then
        Rayfield:Notify({Title="Error", Content="Gagal memuat file: "..fileName, Duration=3, Image="ban"})
        return
    end

    local data = loadCheckpoint(fileName)
    if not data or #data == 0 then
        Rayfield:Notify({Title="Error", Content="Data checkpoint kosong!", Duration=3, Image="ban"})
        return
    end
    ...
    if areaCheckEnabled and isNear then
        Rayfield:Notify({
            Title = "Auto Walk",
            Content = string.format("Berjalan ke titik awal %s (%.1fm)", fileName, distance),
            Duration = 3,
            Image = "map-pin"
        })
        ...
        walkToStartingPoint(startPos, function(success)
            if success then
                Rayfield:Notify({
                    Title = "Auto Walk",
                    Content = "Mulai playback: "..fileName,
                    Duration = 2,
                    Image = "check"
                })
                ...
                startPlayback(data, function()
                    ...
                    if autoLoopEnabled then
                        ...
                        if nextIndex > #checkpoints then
                            -- Sudah di CP terakhir, balik ke spawnpoint
                            if loopingEnabled then
                                Rayfield:Notify({
                                    Title = "Auto Walk (Loop)",
                                    Content = "Sampai checkpoint terakhir! Kembali ke spawnpoint...",
                                    Duration = 4,
                                    Image = "repeat"
                                })
                                task.wait(2)
                                ...
                                Rayfield:Notify({
                                    Title = "Auto Walk",
                                    Content = "Selesai! Semua checkpoint sudah dilewati.",
                                    Duration = 4,
                                    Image = "check-check"
                                })
                                autoLoopEnabled = false
                            ...
                            -- Masih ada checkpoint berikutnya
                            local nextFile = checkpoints[nextIndex]
                            task.wait(1)
                            moveToCheckpoint(nextFile, nextIndex)
                        end
                    else
                        Rayfield:Notify({
                            Title = "Auto Walk",
                            Content = "Playback selesai (tanpa loop)",
                            Duration = 3,
                            Image = "check-check"
                        })
                    end
                ...
                Rayfield:Notify({
                    Title = "Auto Walk",
                    Content = "Gagal menuju ke titik awal!",
                    Duration = 3,
                    Image = "ban"
                })
            end
        ...
        Rayfield:Notify({
            Title = "Auto Walk",
            Content = string.format("Terlalu jauh (%.1fm) dari area %s!", distance, fileName),
            Duration = 3,
            Image = "alert-circle"
        })
    end
...
local CP1Toggle = AutoWalkTab:CreateToggle({
    ...
    Callback = function(Value)
        if Value then
            moveToCheckpoint("checkpoint_1.json", 2)
        else
            autoLoopEnabled = false
            isManualMode = false
            stopPlayback()
...
local CP2Toggle = AutoWalkTab:CreateToggle({
    ...
    Callback = function(Value)
        if Value then
            moveToCheckpoint("checkpoint_2.json", 3)
        else
            autoLoopEnabled = false
            isManualMode = false
            stopPlayback()
...
local CP3Toggle = AutoWalkTab:CreateToggle({
    ...
    Callback = function(Value)
        if Value then
            moveToCheckpoint("checkpoint_3.json", 4)
        else
            autoLoopEnabled = false
            isManualMode = false
            stopPlayback()
...
local CP4Toggle = AutoWalkTab:CreateToggle({
    ...
    Callback = function(Value)
        if Value then
            moveToCheckpoint("checkpoint_4.json", 5)
        else
            autoLoopEnabled = false
            isManualMode = false
            stopPlayback()
...
local CP5Toggle = AutoWalkTab:CreateToggle({
    ...
    Callback = function(Value)
        if Value then
            moveToCheckpoint("checkpoint_5.json", 6)
        else
            autoLoopEnabled = false
            isManualMode = false
            stopPlayback()
...
local CP6Toggle = AutoWalkTab:CreateToggle({
    ...
    Callback = function(Value)
        if Value then
            moveToCheckpoint("checkpoint_6.json", 7)
        else
            autoLoopEnabled = false
            isManualMode = false
            stopPlayback()
...
local CP7Toggle = AutoWalkTab:CreateToggle({
    ...
    Callback = function(Value)
        if Value then
            moveToCheckpoint("checkpoint_7.json", 8)
        else
            autoLoopEnabled = false
            isManualMode = false
            stopPlayback()
...
local CP8Toggle = AutoWalkTab:CreateToggle({
    ...
    Callback = function(Value)
        if Value then
            moveToCheckpoint("checkpoint_8.json", 9)
        else
            autoLoopEnabled = false
            isManualMode = false
            stopPlayback()
...
local CP9Toggle = AutoWalkTab:CreateToggle({
    ...
    Callback = function(Value)
        if Value then
            moveToCheckpoint("checkpoint_9.json", 10)
        else
            autoLoopEnabled = false
            isManualMode = false
            stopPlayback()
...
local CP10Toggle = AutoWalkTab:CreateToggle({
    ...
    Callback = function(Value)
        if Value then
            moveToCheckpoint("checkpoint_10.json", 11)
        else
            autoLoopEnabled = false
            isManualMode = false
            stopPlayback()
...
local CP11Toggle = AutoWalkTab:CreateToggle({
    ...
    Callback = function(Value)
        if Value then
            moveToCheckpoint("checkpoint_11.json", 12)
        else
            autoLoopEnabled = false
            isManualMode = false
            stopPlayback()
...
local CP12Toggle = AutoWalkTab:CreateToggle({
    ...
    Callback = function(Value)
        if Value then
            moveToCheckpoint("checkpoint_12.json", 13)
        else
            autoLoopEnabled = false
            isManualMode = false
            stopPlayback()
...
local CP13Toggle = AutoWalkTab:CreateToggle({
    ...
    Callback = function(Value)
        if Value then
            moveToCheckpoint("checkpoint_13.json", 14)
        else
            autoLoopEnabled = false
            isManualMode = false
            stopPlayback()
...
local CP14Toggle = AutoWalkTab:CreateToggle({
    ...
    Callback = function(Value)
        if Value then
            moveToCheckpoint("checkpoint_14.json", 15)
        else
            autoLoopEnabled = false
            isManualMode = false
            stopPlayback()
...
local CP15Toggle = AutoWalkTab:CreateToggle({
    ...
    Callback = function(Value)
        if Value then
            moveToCheckpoint("checkpoint_15.json", 16)
        else
            autoLoopEnabled = false
            isManualMode = false
            stopPlayback()
...
local CP16Toggle = AutoWalkTab:CreateToggle({
    ...
    Callback = function(Value)
        if Value then
            moveToCheckpoint("checkpoint_16.json", 17)
        else
            autoLoopEnabled = false
            isManualMode = false
            stopPlayback()
...
local CP17Toggle = AutoWalkTab:CreateToggle({
    ...
    Callback = function(Value)
        if Value then
            moveToCheckpoint("checkpoint_17.json", 18)
        else
            autoLoopEnabled = false
            isManualMode = false
            stopPlayback()
